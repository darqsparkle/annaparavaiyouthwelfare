...
export default function Activities() {
  ...
  // Live-subscribe to the admin-managed gallery collection.
  useEffect(() => {
  ...
  const items = images.map((image) => ({
    ...
    title: (isTamil ? image.titleTa : image.titleEn) || image.titleEn,
    description: (isTamil ? image.descriptionTa : image.descriptionEn) || image.descriptionEn,
  }));
      ...
      <section className="bg-bg py-16 lg:py-24">
        <div className="mx-auto max-w-7xl px-5 lg:px-8">
          {loading ? (
            ...
            <p className="text-center text-sm text-dark/50">
              {t("activities.empty", "Photos coming soon.")}
            </p>
          ) : (
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4 lg:gap-6">
              {items.map((item, i) => (
